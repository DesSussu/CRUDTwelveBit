...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Linea para actualizar los datos del empleado
    $sql = "UPDATE empleados SET Nombre='$nombre', Apellidos='$apellidos', Email='$email', Telefono='$telefono' WHERE Cod_Empleado=$id";

    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Empleado actualizado exitosamente');
            location.assign('index.php'); </script>";
    } else {
        echo "<script>alert('Error al actualizar el empleado');
            location.assign('index.php'); </script>";
    }
...
<body class="bg-azul overflow-hidden">
    <div class="w-100">
        ...
        <div class="row p-4 d-flex justify-content-center">
            ...
            <h1 class="fs18 bold fsMono mb-4">Editar Empleado</h1>
            <?php if ($row) { ?>
            <form action="editarEmpleados.php" method="POST" class="d-flex flex-column align-items-center mt-1">
                <input type="hidden" name="id" value="<?php echo $row['Cod_Empleado']; ?>">
                <div>
                ...
                <button type="submit" class="boton mt-5 w-100">Actualizar Empleado</button>
